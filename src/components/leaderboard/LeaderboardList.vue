...
const props = defineProps({
	...
	// Optional overrides for copy, kept minimal
...
function normalizeEntry(entry, idx) {
	...
	const player = entry?.username ?? entry?.user ?? entry?.name ?? entry?.player ?? "Anonymous";
	const rawTime =
		props.variant === "weekly"
			? entry?.average ?? entry?.avg ?? entry?.avgTime ?? entry?.time
			: entry?.time ?? entry?.solveTime ?? entry?.avgTime ?? entry?.average;
...
<template>
	<BCard class="board-card h-100">
		...
		<div v-else>
			...
			<p v-if="!displayItems.length" class="muted text-center my-3">{{ resolvedEmptyText }}</p>
		</div>
		<slot name="footer" />
	</BCard>
</template>
